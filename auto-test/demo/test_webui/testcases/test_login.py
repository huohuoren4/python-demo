...
from demo.test_webui.initial import s_driver, webui_log, sleep_debug
...
login_obj = LoginPage(driver=s_driver, log=webui_log, sleep_debug=sleep_debug)
...
class TestLogin:
    ...
    def test_login_username_valid02(self, v_data):
        """登录页面_用户名的校验01: 逆向"""
        login_obj.login_form(form_datas=common_testcase_data["登录页面"]["登录页面_用户名的校验02"]["表单数据"],
                             v_data=v_data)
        assert login_obj.query_error_tip(v_data[1]["assert_text"]), "登录页面用户名的校验失败, 验证用户名: " + \
                                                                    v_data[0]["v_char"]
